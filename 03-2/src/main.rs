...
fn main() {
    ...
    let mut last_saw_do = true;

    'main: loop {
        if search_iter.clone().next_chunk::<4>().unwrap_or_default() == ['d', 'o', '(', ')'] {
            last_saw_do = true;
            let _ = search_iter.next_chunk::<4>();
        }

        if search_iter.clone().next_chunk::<7>().unwrap_or_default()
            == ['d', 'o', 'n', '\'', 't', '(', ')']
        {
            last_saw_do = false;
            let _ = search_iter.next_chunk::<7>();
        }
